...
class AboutMonkeyPatching(Koan):
    ...
    def test_subclasses_of_built_in_classes_can_be_be_monkey_patched(self):
        ...
        hack = "extending via subclassing is a Python hack but not in Kotlin"
        self.assertEqual(False, self.MyInt(1).is_even(), hack)
        self.assertEqual(True, self.MyInt(2).is_even(), hack)
